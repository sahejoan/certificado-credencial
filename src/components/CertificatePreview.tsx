...
export default function CertificatePreview({ event, participant, authorities, onClose }: CertificatePreviewProps) {
  ...
  useEffect(() => {
    ...
    loadSignatures();
  }, [template.elements, event.authorities, authorities]);
  ...
  const handleDownload = () => {
    ...
    link.download = `Certificado_${participant.name.replace(/\s+/g, '_')}.png`;
    link.href = uri;
  ...
  return (
    ...
    >
      <div className="bg-zinc-900 border border-white/10 rounded-3xl shadow-2xl overflow-hidden max-w-5xl w-full flex flex-col" onClick={e => e.stopPropagation()}>
